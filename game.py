...
def remove_punct(text):
    ...
    """
    punctless= ""
    #above is variable that will hold string with removed punctuation

    for char in text: # runs through each character in the text string   
    	if char not in string.punctuation: # if the character is punctuation, checked against list of punctuation held in python 
    		punctless += char # if the above statement is true, the character is added to the string held in punctless

    return punctless #function returns the variable punctless
...
def remove_spaces(text):
    ...
    ''
    """
    text = text.strip() # uses the python standard library fuction of strip to remove whitespace
    # could have created for loops to go through checking each character in the text , but this would create more code and be less efficient
    return text  # function then returns text, which has been stripped of spaces  
...
def normalise_input(user_input):
    ...
    # this will return the normalised version of the user input by calling the spaces, punct functions
    # then also putting in lower case using .lower    
def display_room(room):
    ...
    """
    print() #blankline printed
    print(str.upper(room["name"])) #prints room name, .upper puts in caps
    print() #blank line printed
    print(room["description"]) #prints room description
    print() #blank line printed
...
def print_menu_line(direction, leads_to):
    ...
    """
    print("Go " + str.upper(direction) + " to " + leads_to + ".") # prints the string go, then in upper case the direction then the leads_to followed by . 
...
def print_menu(exits):
    ...
    for direction in exits: # for all avaialbe exits printed
            print_menu_line(direction,exit_leads_to(exits,direction)) #prints out the direction and name of exit from above variable
    # COMPLETE THIS PART:
    # Iterate over available exits:
    #     and for each exit print the appropriate menu line

    print("Where do you want to go?") #prints out where you want to go 
...
def is_valid_exit(exits, user_input):
    ...
    if exits.get(user_input): # if exit user entered is in the exits list
        return True # if yes return true
    else:
        return False # if not function  returns false , and in menu will reject the input and ask for another
...
def menu(exits):
    ...
    while True:
       print_menu(exits) #prints available exits

       
       user_input=str(input()) # sets the user input variable as an input field , also defining as string
       user_input=normalise_input(user_input) # runs the normalise function on the input so can be read by system

       if is_valid_exit(exits, user_input): # uses valid exit function to check if the value entered is possible

       		return user_input #function then returns the user input , normalised and valid.
        # COMPLETE       
        # Display menu
...
def move(exits, direction):
    ...
    False
    """
    return rooms[exits[direction]] # uses dictionary to give the room name 
